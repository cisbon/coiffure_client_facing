-- ============================================================
-- Migration 022: Subscription plans, salon subscriptions, invoices
-- ============================================================
-- Administrator-only bookkeeping. There is deliberately NO payment gateway
-- integration: payment_status is set by hand and invoices are rendered as a
-- printable page, not charged.
--
-- coiffure_subscription_plans
--   features is a JSON object of boolean feature flags, e.g.
--   {"advanced_insights":true,"white_label":false,"multi_salon":true}. Kept as
--   JSON so adding a feature does not need a migration; the dashboard renders
--   a checkbox per known key.
--   max_customers / max_campaigns_per_month: 0 means unlimited.
--
-- coiffure_salon_subscriptions
--   One current subscription per salon (UNIQUE on salon_id).
--   payment_status: active | overdue | cancelled -- tracked manually.
--
-- coiffure_invoices
--   invoice_number is a human-facing running number ("2026-0001") generated in
--   api/billing.php inside a transaction; the surrogate invoice_id stays the
--   primary key. Line items live in coiffure_invoice_items so extra usage
--   charges can be added alongside the plan fee.
--
-- Safe to run once. Use api/apply_migration_022.php for an idempotent runner.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS coiffure_subscription_plans (
    ...
    max_customers INT UNSIGNED NOT NULL DEFAULT 0,           -- 0 = unlimited
    max_campaigns_per_month INT UNSIGNED NOT NULL DEFAULT 0, -- 0 = unlimited
    features TEXT DEFAULT NULL,                              -- JSON object
...
-- Starter plans so the Abrechnung view is not empty on first load.
